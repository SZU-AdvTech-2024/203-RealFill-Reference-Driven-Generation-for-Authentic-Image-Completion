...
#用于模型验证，其主要工作是对模型生成能力的测试，主要步骤包括加载验证数据、推理生成图像
# 在训练过程中周期性检查模型生成的图像质量。
@torch.no_grad()
def log_validation(
    text_encoder,
    tokenizer,
...
#parse_args 会根据命令行输入或传入的 input_args 解析参数。
#析过程中会处理一些环境变量，特别是涉及到分布式训练时（例如 local_rank 参数）。
#返回解析后的参数对象（args），包含所有配置选项。
def parse_args(input_args=None):
    ...
    # 当输入参数不为空，则使用parse_args对其进行处理并赋值。
    # args 是一个命名空间（argparse.Namespace）对象，里面包含了所有解析出的参数
    if input_args is not None:
        args = parser.parse_args(input_args)
    else:
        args = parser.parse_args()
    # 获取环境变量 LOCAL_RANK 的值。这个值通常用于分布式训练中，标识每个进程的本地设备编号。
...
# 这个类主要用于为模型训练准备数据集，包括加载图像、生成遮罩（mask）、进行数据增强、以及对文本提示（prompt）进行标记化。
# 它的目标是为 Fine-tuning 过程提供训练所需的图像和相应的遮罩。
# 其返回输入的图像、mask、权重图像、条件图像（遮罩之后的图像）以及文本prompt。
...
# 批处理函数，主要用于将数据处理成适合模型训练的格式。
'''
接收从数据集中获取的多个样本（一个批次的数据）。
对这些样本中的各个字段（如图像、遮罩、权重、文本等）进行堆叠和转换，使它们变成适合模型训练的格式。
返回一个包含所有字段的字典，这个字典将作为一个批次的输入数据传递给模型。
'''
def collate_fn(examples):
    ...
    '''
    将 images 列表中的图像张量堆叠成一个批次。
    堆叠后的张量的形状是 (batch_size, C, H, W)
    其中 batch_size 是批次大小，C 是通道数（通常为 3，表示 RGB 图像），H 和 W 是图像的高度和宽度。
    '''
    images = torch.stack(images)
    # 图像张量转换为连续的内存格式，并转换为浮点类型
    ...
    '''
    将 input_ids 按 dim=0 维度拼接，意味着将所有样本的文本输入（标记化后的文本）合并成一个张量，形状为 (batch_size, sequence_length)，
    其中 batch_size 是批次的大小，sequence_length 是每个文本序列的长度。
    '''
...
# MAIN
'''
负责执行模型的训练过程，包括模型加载、优化器设置、调度器配置、数据加载、训练过程的迭代等。
它还集成了混合精度训练、梯度累积、模型检查点保存、模型推送到 Hugging Face Hub 等功能。
————————————————————————
其基本过程为
- 初始化训练环境（accelerator）
- 加载组件（tokenizer、noise_scheduler 、text_encoder、vae、nuet）
- 为unet、text_encoder设置LoRA优化
- 配置优化器
- 创建数据加载器，即将载入的，用于训练的数据集以及相关参数作为输入，创建一个DataLoader实例，便于进行训练学习；
- 创建一个学习率调度器，动态调整学习率以适应训练过程的需求。
- 训练循环
- 训练完成后保存训练模型。
'''
def main(args):
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet, text_encoder):
                ...
                #这里vae得到的latents尺寸会降低，故后面也需要对mask尺寸降低。
                #乘积是为了保证其噪声范围在可控制范围内，此处可见diffusers的教程。
            ...
            if accelerator.sync_gradients:
                ...
                # 只有主进程需要执行的操作（如保存检查点和验证），以避免多 GPU 或分布式环境下重复操作。
                if accelerator.is_main_process:
                    if global_step % args.checkpointing_steps == 0:
                        # _before_ saving state, check if this save would set us over the `checkpoints_total_limit`
                        # 当设定了允许保存的checkpoints数量上限。
